...
class ReviewController {
    ...
    //투두 리뷰작성
    todoReviews = async (req,res)=>{
        const {access_token} = req.cookies;
        const {title,name,date,myComment} = req.body;
        await this.reviewService.todoReviews(access_token,title,name,date,myComment);
        return res.status(201).json({msg:"댓글작성 완료"});
    }
    //리뷰조회
    getTodoReviews =async (req,res)=>{
        const {title,date,name} = req.query;
        const todoReviews = await this.reviewService.getTodoReviews(title,name,date);
        return res.status(200).json(todoReviews);
    }
    delTodoReviews =async (req,res)=>{
        const {access_token} = req.cookies;
        const {title,name,date,content} = req.body;
        await this.reviewService.delTodoReviews(access_token,title,name,date,content);
        return res.status(200).json({msg:'댓글 삭제 완료'});
    }
